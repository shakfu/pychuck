...
- **Chugins were unloaded while their own threads were still running** (`thirdparty/chuck/core/chuck_dl.cpp`, `scripts/patches/0004-chugin-rtld-nodelete.patch`): `Chuck_DLL::load()` called `dlopen()` without `RTLD_NODELETE`, so `~Chuck_DLL()` genuinely unmapped a chugin's text. Every chugin is deleted by `Chuck_ImportRegistry::shutdown()`, which `ChucK::shutdown()` reaches via `CK_SAFE_DELETE(m_carrier->compiler)` -- but nothing guarantees the objects a chugin created are already gone by then, and a chugin may have started a thread of its own. ConvRev does: `ConvRev::tick()` spawns a `std::thread` per FFT block whose only joins are the next block boundary and `~ConvRev()`, so the last block's worker outlives `chuck.run()` and can still be inside `FFTConvolver::process()` when the unmap lands. The next instruction fetch then faults. This was the "intermittent SIGSEGV on a background thread during ConvRev playback" tracked in `TODO.md`, at 7 crashes in 152 runs of `pytest tests/test_api.py tests/test_examples.py`. Seven captured faults were identical and unambiguous: `SEGV_MAPERR` with `si_addr == RIP`, the faulting thread never the main one, `ConvRev.chug` absent from `/proc/self/maps` while ~20 sibling chugins remained mapped, and both `RIP` and the return addresses on that thread's stack sitting in the hole it left -- resolving to `.text+0xcda9`, `.text+0xcfcd` and `.text+0xcabe`, that is `ComplexMultiplyAccumulate` called from `ComplexMultiplyAccumulate` called from `FFTConvolver::process`. Chugins are now opened with `RTLD_NODELETE`, and the Windows `dlopen` shim -- which ignored `mode` entirely -- honors it by pinning the module, since `FreeLibrary()` has the same hazard. Not unmapping costs only address space, and unloading plugin code that can outlive its own unload is unsafe in general, which is why CPython never `dlclose`s extension modules either. Verified over 400 runs of the reproducer.

- **`faulthandler`'s re-raise made every crash address look like a corrupted pointer**: worth recording because it sent the previous investigation down a blind alley. CPython's fatal-signal handler prints its traceback and then calls `raise(signum)`, which glibc implements as `tgkill()`, so anything downstream -- a chained handler, AddressSanitizer -- sees `si_code == SI_TKILL` and an `si_addr` that is not a fault address at all but the `si_pid`/`si_uid` pair overlaid on the same union. That is the whole story behind the `SEGV on unknown address 0x03e80002964c` recorded in `TODO.md`: `0x3e8` is uid 1000 and `0x2964c` is pid 169548. It was never a corrupted pointer. Running with `-p no:faulthandler` and installing a handler ahead of it yields the real `SEGV_MAPERR` and a usable `RIP`.

- **The OTF listener thread outlived the VM and segfaulted** (`thirdparty/chuck/core/chuck.cpp`, `util_network.c`, `scripts/patches/0002-otf-thread-join.patch`): `ChucK::shutdown()` called `pthread_cancel` on the on-the-fly command thread and then abandoned it -- there is no `pthread_join` anywhere in the file. Cancellation only takes effect at a cancellation point, so `otf_recv_cb()` kept running while shutdown freed `otf_socket` and went on to tear down the VM, the compiler and the carrier, all of which that thread dereferences. It surfaced as an intermittent SIGSEGV in whatever test happened to be running, roughly one full-suite run in seven, which read as a flaky chugin test rather than a shutdown bug. gdb put it beyond doubt: the faulting instruction is `cmpl $0x1,0x4(%rdi)` at `ck_accept+33` -- that is `sock->prot` -- with `%rdi` zero and a fault address of `0x4`, so the thread had read `carrier->otf_socket` back after shutdown nulled it. The loop checks the shutdown flag only *after* calling `ck_accept`, so it could not notice in time. Shutdown now clears the flag first, cancels, and joins before freeing anything the thread can reach; `ck_accept()` also gained the NULL guard its sibling `ck_close()` already had. The trigger in-tree is `tests/test_api.py`, which constructs a VM with `otf_enable=True` and never shuts it down, so teardown happens at GC time -- arbitrarily later, which is what made it look unrelated to OTF. Verified over 100 runs of the reproducer: no OTF crash, no deadlock (the real risk when joining a cancelled thread), and no change in suite runtime.

- **Audio buffers allocated with `new[]` were freed with scalar `delete`** (`thirdparty/chuck/host/chuck_audio.cpp`, `scripts/patches/0003-audio-buffer-delete-array.patch`): `m_buffer_in` and `m_buffer_out` are allocated as `new SAMPLE[...]` in `ChuckAudio::initialize()` but released with `CK_SAFE_DELETE` rather than `CK_SAFE_DELETE_ARRAY`, so every audio teardown ended in undefined behaviour on a 4096-byte allocation. Found with an AddressSanitizer build, which names it exactly: `alloc-dealloc-mismatch (operator new [] vs operator delete)`, allocated at `chuck_audio.cpp:1121`, freed at `chuck_audio.cpp:1313`. The other `CK_SAFE_DELETE` calls in that file are on scalars and are correct.

  This did not fix the remaining crash it was suspected of causing. An intermittent segfault survives at roughly 3-5 runs in 100, always while `tests/test_examples.py::test_chugin_convrev_example` is executing. ASAN caught the signal itself -- `SEGV on unknown address 0x03e80002964c ... T107`, a read through a corrupted pointer on a *background* thread, not the main one -- but its reporter died before printing frames (`nested bug in the same thread, aborting`), and the race does not reproduce under gdb. The same ASAN run also reports a genuine stack over-read in the GVerb chugin (`gverbdefs.h:114`, an 8-byte read of a 4-byte stack float, ten occurrences per run); it is real undefined behaviour but a read of adjacent stack, so it does not explain a segfault and is left unfixed. Tracked in `TODO.md`.

- **NHHall did not compile on GCC 15** (`thirdparty/chugins/NHHall/nh_hall.hpp`, `scripts/patches/chugins/0001-nhhall-cstdint.patch`): the header uses `uint16_t` and `uint32_t` without including `<cstdint>`, and built only because older libstdc++ pulled that header in transitively via `<cstdlib>`/`<memory>`. libstdc++ 15 no longer does, so the build failed outright with `'uint32_t' has not been declared`. The include is now explicit. Since this is vendored upstream code that `scripts/update.sh` overwrites wholesale, the fix is carried as the first entry in a new `scripts/patches/chugins/` directory -- which incidentally repairs a latent bug in `apply_patches`, whose unguarded `*.patch` glob left the pattern literal and failed `git apply` while that directory did not exist.
...
- **The bison/flex parser is generated into the build tree, not the source tree** (`thirdparty/chuck/core/CMakeLists.txt`): the generation step ran with `WORKING_DIRECTORY ${CMAKE_CURRENT_SOURCE_DIR}`, so every CMake configure rewrote the committed `chuck.tab.c`, `chuck.tab.h`, `chuck.yy.c` and `chuck.output` in place. Any build on a machine whose bison differs from the one that produced the committed files left the vendored tree permanently dirty -- upstream generated them with bison 2.3, so a modern bison restates roughly 3,900 lines -- and that in turn blocked `scripts/update.sh`, which refuses to update a tree with uncommitted changes to upstream-owned files. Output now goes to `${CMAKE_CURRENT_BINARY_DIR}/parser`, and the committed copies stay in `core/` as the fallback for hosts without bison or flex; that path was verified by forcing the tools to be missing. Three details went with it: the generated sources still `#include "chuck_absyn.h"`, which no longer resolves relative to the including file, so `core/` joined the target's include path; the macOS `-Wno-shorten-64-to-32` suppression matched `chuck.yy.c` by relative name and would have silently stopped applying, so it follows the path variable now; and the two tools were passed as two `COMMAND`s to a single `execute_process`, which *pipes* them rather than sequencing them and hid each tool's exit status, so they are now separate calls with their results checked.
...
- **A tap test measured wall-clock time and assumed a frame count** (`tests/test_ugen_tap.py`): `test_tap_keeps_more_history_than_one_block` slept 0.5s and asserted the whole 8192-frame window was captured audio, on the reasoning that 0.5s at 44100 Hz is far more than 8192 frames. That holds on hardware, which is clocked to real time, and not on a virtual device: CI's PulseAudio null sink delivers audio but more slowly, so the oldest part of the window was still zero padding and the test failed. It now polls until the window actually fills -- which is the property being claimed, stated directly rather than inferred from a sleep -- and skips with the measured frame count if the device is too slow. The same reasoning applies to a loaded machine with real hardware, so this is more robust everywhere, not only on CI.

- **The tap regression test could pass on silence** (`tests/test_ugen_tap.py`): `test_realtime_tap_reads_are_never_torn` asserts that no sample-to-sample step exceeds the sine's own maximum -- which an all-zero buffer satisfies trivially, since `np.diff` of silence is silence. Running it in CI for the first time made that visible: the runner's audio device opens and then delivers nothing, and the test went green having verified no property at all. It now also asserts the tap carried an audible signal, and the tests that need real audio go through a `require_audible_tap()` helper that waits for signal and skips with an explicit reason if none arrives. Verified both ways: the four tap tests pass against real hardware, and the helper skips against a deliberately silenced UGen.
...
- **Four latent CI defects, exposed by turning CI back on** (`.github/workflows/ci.yml`, `pyproject.toml`): re-enabling the workflows surfaced problems that had been invisible while nothing ran.
  ...
  *An audio device the runners cannot provide.* The `realtime` job loaded `snd-dummy`, which the Azure runner images have no kernel modules for. It now tries `snd-dummy`, falls back to a PulseAudio null sink (no kernel support needed), and reports which route worked. If neither is available it emits a warning annotation and skips the test step rather than failing the build on a runner limitation -- loud about not having tested rather than quietly appearing to. Setting the null sink as the default output was what made it actually carry audio; with that in place the tap tests, including the seqlock tear check, run for real on the runner rather than skipping.

  *Lint that only reproduced on the runner, again.* The lint job installed ruff unpinned, so it ran whatever version had shipped that day -- 0.16.3 against the locked 0.14.10 -- and the formatter is not stable across minor versions. `ruff format --check` failed on a lambda that the local, locked ruff leaves alone. The job now installs from the lockfile (`uv sync --only-group dev --no-install-project`), so the version is the same one contributors have.
