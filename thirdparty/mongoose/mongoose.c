...
int mg_base64_decode(const char* src, int n, char* dst)
{
    ...
    while (src != NULL && src + 3 < end) {
        int a = mg_b64rev(src[0]), b = mg_b64rev(src[1]),
            c = mg_b64rev(src[2]), d = mg_b64rev(src[3]);
                         ...
                         struct mg_dns* dnsc, bool ipv6)
{
    ...
    if (dnsc->c == NULL) {
        ...
        MG_VERBOSE(("%lu resolving %.*s @ %s, txnid %hu", c->id,
                    (int)name->len, name->ptr, dnsc->url, d->txnid));
...
{
    size_t i, n = 0;
    const char* t = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz01234"
                    "56789+/";
    out('"', ptr), n++;
    for (i = 0; i < len; i += 3) {
                        ...
                        const struct mg_http_serve_opts* opts)
{
    ...
    if (fd == NULL || fs->st(path, &size, &mtime) == 0) {
        ...
        mg_printf(c,
                  ...
                  mime.ptr, etag, cl, gzip ? "Content-Encoding: gzip\r\n" : "",
                    ...
                    const struct mg_http_serve_opts* opts, char* dir)
{
    const char* sort_js_code = "<script>function srt(tb, sc, so, d) {"
                               "var tr = Array.prototype.slice.call(tb.rows, "
                               "0),"
                               "tr = tr.sort(function (a, b) { var c1 = "
                               "a.cells[sc], c2 = b.cells[sc],"
                               "n1 = c1.getAttribute('name'), n2 = "
                               "c2.getAttribute('name'), "
                               "t1 = a.cells[2].getAttribute('name'), "
                               "t2 = b.cells[2].getAttribute('name'); "
                               "return so * (t1 < 0 && t2 >= 0 ? -1 : t2 < 0 "
                               "&& t1 >= 0 ? 1 : "
                               "n1 ? parseInt(n2) - parseInt(n1) : "
                               "c1.textContent.trim().localeCompare(c2."
                               "textContent.trim())); });";
    const char* sort_js_code2 = "for (var i = 0; i < tr.length; i++) "
                                "tb.appendChild(tr[i]); "
                                "if (!d) window.location.hash = ('sc=' + sc + "
                                "'&so=' + so); "
                                "};"
                                ...
                                "var m = "
                                "/sc=([012]).so=(1|-1)/"
                                ".exec(window.location.hash) || [0, 2, 1];"
                                "var sc = m[1], so = m[2]; document.onclick = "
                                "function(ev) { "
                                "var c = ev.target.rel; if (c) {if (c == sc) "
                                "so *= -1; srt(tb, c, so); "
                        ...
                        char* path, size_t path_size)
{
    ...
    MG_VERBOSE(("%lu %.*s -> %s %d", c->id, (int)hm->uri.len, hm->uri.ptr,
                path, flags));
    if (flags == 0) {
        ...
        while (c->recv.buf != NULL && c->recv.len > 0) {
            ...
            if (ev == MG_EV_CLOSE) { // If client did not set Content-Length
                ...
                hm.body.len = hm.message.len
                    - (size_t)(hm.body.ptr - hm.message.ptr);
            }
            if (mg_is_chunked(&hm)) {
...
int mg_iobuf_resize(struct mg_iobuf* io, size_t new_size)
{
    ...
    if (new_size == 0) {
        ...
        // the porting to some obscure platforms like FreeRTOS
...
void mg_mqtt_login(struct mg_connection* c, const struct mg_mqtt_opts* opts)
{
    ...
    if (opts->will_topic.len > 0 && opts->will_message.len > 0) {
        total_len += 4 + (uint32_t)opts->will_topic.len
            + (uint32_t)opts->will_message.len;
        hdr[7] |= MQTT_HAS_WILL;
    }
...
{
    struct mg_connection* c = (struct mg_connection*)calloc(
...
size_t mg_print_ip(void (*out)(char, void*), void* arg, va_list* ap)
{
    ...
    if (addr->is_ip6)
        return print_ip6(out, arg, (uint16_t*)addr->ip6);
...
bool mg_open_listener(struct mg_connection* c, const char* url)
{
    ...
    if (!mg_aton(mg_url_host(url), &c->loc)) {
     ...
     || (defined(LWIP_SOCKET) && SO_REUSE == 1))
                                    ...
                                    sizeof(on)))
                   != 0) {
            // 1. SO_RESUSEADDR is not enabled on Windows because the semantics
            // of
            ...
            //    hard-to-track failure scenarios. Therefore, SO_REUSEADDR was
            //    disabled on Windows unless SO_EXCLUSIVEADDRUSE is supported
            //    and set on a socket.
            // 2. In case of LWIP, SO_REUSEADDR should be explicitly enabled,
            // by defining
            //    SO_REUSE (in lwipopts.h), otherwise the code below will
            //    compile but won't work! (setsockopt will return EINVAL)
...
void mg_connect_resolved(struct mg_connection* c)
{
    ...
    if (FD(c) == MG_INVALID_SOCKET) {
        ...
        if (rc == 0) {                       // Success
            ...
            MG_DEBUG(("%lu %p -> %M pend", c->id, c->fd, mg_print_ip_port,
                      &c->rem));
            c->is_connecting = 1;
        } else {
...
static void accept_conn(struct mg_mgr* mgr, struct mg_connection* lsn)
{
    ...
    if (fd == MG_INVALID_SOCKET) {
        ...
        // readable even it is not. See comment for 'select' func
        // implementation in nx_bsd.c That's not an error, just should try
        // later
        if (errno != EAGAIN)
#endif
            MG_ERROR(
...
int64_t mg_to64(struct mg_str str)
{
    int64_t result = 0, neg = 1,
            max = 922337203685477570 /* INT64_MAX/10-10 */;
    size_t i = 0;
    while (i < str.len && (str.ptr[i] == ' ' || str.ptr[i] == '\t'))
        i++;
...
void mg_tls_init(struct mg_connection* c, const struct mg_tls_opts* opts)
{
    ...
    if (opts->ca != NULL && opts->ca[0] != '\0') {
        SSL_set_verify(
            tls->ssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, NULL);
...
static size_t ws_process(uint8_t* buf, size_t len, struct ws_msg* msg)
{
    ...
    // Sanity check, and integer overflow protection for the boundary check
    // below data_len should not be larger than 1 Gb
...
struct imx_rt1020_enet {
    volatile uint32_t RESERVED0, EIR, EIMR, RESERVED1, RDAR, TDAR,
        RESERVED2[3], ECR, RESERVED3[6], MMFR, MSCR, RESERVED4[7], MIBC,
        RESERVED5[7], RCR, RESERVED6[15], TCR, RESERVED7[7], PALR, PAUR, OPD,
        TXIC0, TXIC1, TXIC2, RESERVED8, RXIC0, RXIC1, RXIC2, RESERVED9[3],
        IAUR, IALR, GAUR, GALR, RESERVED10[7], TFWR, RESERVED11[14], RDSR,
        TDSR, MRBR[2], RSFL, RSEM, RAEM, RAFL, TSEM, TAEM, TAFL, TIPG, FTRL,
        RESERVED12[3], TACC, RACC, RESERVED13[15], RMON_T_PACKETS,
        ...
        IEEE_T_OCTETS_OK, RESERVED14[3], RMON_R_PACKETS, RMON_R_BC_PKT,
        ...
        ATINC, ATSTMP, RESERVED17[122], TGSR, TCSR0, TCCR0, TCSR1, TCCR1,
        TCSR2, TCCR2, TCSR3;
};
...
{
    ENET->EIR |= BIT(23);             // MII interrupt clear
    uint32_t mask_phy_adr_reg = 0x1f; // 0b00011111: Ensure we write 5 bits
...
{
    ENET->EIR |= BIT(23);            // MII interrupt clear
    uint8_t mask_phy_adr_reg = 0x1f; // 0b00011111: Ensure we write 5 bits (Phy
                                     // address & register)
...
static bool mg_tcpip_driver_imxrt1020_init(struct mg_tcpip_if* ifp)
{
    ...
    ENET->MSCR = 0x130; // HOLDTIME 2 clk, Preamble enable, MDC MII_Speed Div
                        // 0x30
    ...
    // PHY: Start Link
    {
        ...
        uint32_t bcr = imxrt1020_eth_read_phy(IMXRT1020_PHY_ADDR,
                                              IMXRT1020_PHY_BCR);
    ...
    // Configure ENET
    ENET->RCR = 0x05ee0104; // #CRCFWD=0 (CRC kept in frame) + RMII + MII
                            // Enable
    ...
    for (int i = 0; i < ENET_RXBD_NUM; i++) {
        ...
        rx_buffer_descriptor[i].control = (BIT(15)
                                           | ((i < (ENET_RXBD_NUM - 1))
                                                  ? 0
                                                  : BIT(13))); // E+(W*)
    ...
    for (int i = 0; i < ENET_TXBD_NUM; i++) {
        // Wrap last descriptor buffer ptr
        tx_buffer_descriptor[i].control = ((i < (ENET_RXBD_NUM - 1)) ? 0
                                                                     : BIT(13))
            | BIT(10); // (W*)+TC
                                           ...
                                           struct mg_tcpip_if* ifp)
{
    ...
    if (len > sizeof(tx_data_buffer[ENET_TXBD_NUM])) {
        ...
        tx_buffer_descriptor[s_rt1020_txno].length = (uint16_t)
            len; // Set data len
        tx_buffer_descriptor[s_rt1020_txno].control |= (uint16_t)(BIT(
            10)); // TC (transmit CRC)
        ...
        //                           11(L)  last in frame
        //                           10(TC) transmis CRC
        // __DSB(); // ARM errata 838869 Cortex-M4, M4F, M7, M7F: "store
        // immediate overlapping exception" return might vector to incorrect
        // interrupt.
        ...
        // Read inframes
            ...
            for (uint32_t i = 0; i < 10;
                ...
                if (!(rx_buffer_descriptor[s_rt1020_rxno].control
                      & (BIT(2) | BIT(0)))) {
                    uint32_t len = (rx_buffer_descriptor[s_rt1020_rxno]
                                        .length);
                    mg_tcpip_qwrite(rx_buffer_descriptor[s_rt1020_rxno].buffer,
                                    len > 4 ? len - 4 : len, s_ifp);
                }
                rx_buffer_descriptor[s_rt1020_rxno].control |= BIT(
                    15); // Inform DMA RX is empty
...
static bool mg_tcpip_driver_imxrt1020_up(struct mg_tcpip_if* ifp)
{
    uint32_t bsr = imxrt1020_eth_read_phy(IMXRT1020_PHY_ADDR,
                                          IMXRT1020_PHY_BSR);
    (void)ifp;
...
struct stm32_eth {
    volatile uint32_t MACCR, MACFFR, MACHTHR, MACHTLR, MACMIIAR, MACMIIDR,
        ...
        MACA2LR, MACA3HR, MACA3LR, RESERVED2[40], MMCCR, MMCRIR, MMCTIR,
        MMCRIMR, MMCTIMR, RESERVED3[14], MMCTGFSCCR, MMCTGFMSCCR, RESERVED4[5],
        MMCTGFCR, RESERVED5[10], MMCRFCECR, MMCRFAECR, RESERVED6[10],
        MMCRGUFCR, RESERVED7[334], PTPTSCR, PTPSSIR, PTPTSHR, PTPTSLR,
        PTPTSHUR, PTPTSLUR, PTPTSAR, PTPTTHR, PTPTTLR, RESERVED8, PTPTSSR,
        PTPPPSCR, RESERVED9[564], DMABMR, DMATPDR, DMARPDR, DMARDLAR, DMATDLAR,
        DMASR, DMAOMR, DMAIER, DMAMFBOCR, DMARSWTR, RESERVED10[8], DMACHTDR,
        DMACHRDR, DMACHTBAR, DMACHRBAR;
};
#undef ETH
...
{
    struct mg_tcpip_driver_stm32_data* d = (struct mg_tcpip_driver_stm32_data*)
                                               ifp->driver_data;
    s_ifp = ifp;
    ...
    for (int i = 0; i < ETH_DESC_CNT; i++) {
        ...
        s_rxdesc[i][2] = (uint32_t)(uintptr_t)
            s_rxbuf[i]; // Point to data buffer
        s_rxdesc[i][3] = (uint32_t)(uintptr_t)
            s_rxdesc[(i + 1) % ETH_DESC_CNT]; // Chain
    }

    // Init TX descriptors
    for (int i = 0; i < ETH_DESC_CNT; i++) {
        s_txdesc[i][2] = (uint32_t)(uintptr_t)s_txbuf[i]; // Buf pointer
        s_txdesc[i][3] = (uint32_t)(uintptr_t)
            s_txdesc[(i + 1) % ETH_DESC_CNT]; // Chain
    }
    ...
    // Set MDC clock divider. If user told us the value, use it. Otherwise,
    // guess
    ...
    ETH->MACA0LR = (uint32_t)(ifp->mac[3] << 24)
        | ((uint32_t)ifp->mac[2] << 16) | ((uint32_t)ifp->mac[1] << 8)
        | ifp->mac[0];
    return true;
}
                                       ...
                                       struct mg_tcpip_if* ifp)
{
    if (len > sizeof(s_txbuf[s_txno])) {
        ...
        s_txdesc[s_txno][0] = BIT(20) | BIT(28) | BIT(29)
            | BIT(30);                  // Chain,FS,LS
    ...
    if (ETH->DMASR & BIT(6)) {         // Frame received, loop
        ...
        for (uint32_t i = 0; i < 10;
            ...
            if (s_rxdesc[s_rxno][0] & BIT(31))
                break; // exit when done
            if (((s_rxdesc[s_rxno][0] & (BIT(8) | BIT(9)))
                 == (BIT(8) | BIT(9)))
                && !(s_rxdesc[s_rxno][0]
                     & BIT(15))) { // skip partial/errored frames
...
struct stm32h_eth {
    volatile uint32_t MACCR, MACECR, MACPFR, MACWTR, MACHT0R, MACHT1R,
        ...
        MACLCSR, MACLTCR, MACLETR, MAC1USTCR, RESERVED9[12], MACVR, MACDR,
        RESERVED10, MACHWF0R, MACHWF1R, MACHWF2R, RESERVED11[54], MACMDIOAR,
        MACMDIODR, RESERVED12[2], MACARPAR, RESERVED13[59], MACA0HR, MACA0LR,
        MACA1HR, MACA1LR, MACA2HR, MACA2LR, MACA3HR, MACA3LR, RESERVED14[248],
        MMCCR, MMCRIR, MMCTIR, MMCRIMR, MMCTIMR, RESERVED15[14], MMCTSCGPR,
        MMCTMCGPR, RESERVED16[5], MMCTPCGR, RESERVED17[10], MMCRCRCEPR,
        MMCRAEPR, RESERVED18[10], MMCRUPGR, RESERVED19[9], MMCTLPIMSTR,
        MMCTLPITCR, MMCRLPIMSTR, MMCRLPITCR, RESERVED20[65], MACL3L4C0R,
        MACL4A0R, RESERVED21[2], MACL3A0R0R, MACL3A1R0R, MACL3A2R0R,
        MACL3A3R0R, RESERVED22[4], MACL3L4C1R, MACL4A1R, RESERVED23[2],
        MACL3A0R1R, MACL3A1R1R, MACL3A2R1R, MACL3A3R1R, RESERVED24[108],
        MACTSCR, MACSSIR, MACSTSR, MACSTNR, MACSTSUR, MACSTNUR, MACTSAR,
        RESERVED25, MACTSSR, RESERVED26[3], MACTTSSNR, MACTTSSSR,
        RESERVED27[2], MACACR, RESERVED28, MACATSNR, MACATSSR, MACTSIACR,
        MACTSEACR, MACTSICNR, MACTSECNR, RESERVED29[4], MACPPSCR,
        RESERVED30[3], MACPPSTTSR, MACPPSTTNR, MACPPSIR, MACPPSWR,
        RESERVED31[12], MACPOCR, MACSPI0R, MACSPI1R, MACSPI2R, MACLMIR,
        RESERVED32[11], MTLOMR, RESERVED33[7], MTLISR, RESERVED34[55],
        MTLTQOMR, MTLTQUR, MTLTQDR, RESERVED35[8], MTLQICSR, MTLRQOMR,
        MTLRQMPOCR, MTLRQDR, RESERVED36[177], DMAMR, DMASBMR, DMAISR, DMADSR,
        RESERVED37[60], DMACCR, DMACTCR, DMACRCR, RESERVED38[2], DMACTDLAR,
        RESERVED39, DMACRDLAR, DMACTDTPR, RESERVED40, DMACRDTPR, DMACTDRLR,
        DMACRDRLR, DMACIER, DMACRIWTR, DMACSFCSR, RESERVED41, DMACCATDR,
        RESERVED42, DMACCARDR, RESERVED43, DMACCATBR, RESERVED44, DMACCARBR,
        DMACSR, RESERVED45[2], DMACMFCR;
};
#undef ETH
#define ETH                                                                   \
...
static uint32_t get_hclk(void)
{
    struct rcc {
        volatile uint32_t CR, HSICFGR, CRRCR, CSICFGR, CFGR, RESERVED1, D1CFGR,
            ...
            D2CCIP1R, D2CCIP2R, D3CCIPR, RESERVED4, CIER, CIFR, CICR,
            RESERVED5, BDCR, CSR, RESERVED6, AHB3RSTR, AHB1RSTR, AHB2RSTR,
            AHB4RSTR, APB3RSTR, APB1LRSTR, APB1HRSTR, APB2RSTR, APB4RSTR, GCR,
            RESERVED8, D3AMR, RESERVED11[9], RSR, AHB3ENR, AHB1ENR, AHB2ENR,
            AHB4ENR, APB3ENR, APB1LENR, APB1HENR, APB2ENR, APB4ENR, RESERVED12,
            AHB3LPENR, AHB1LPENR, AHB2LPENR, AHB4LPENR, APB3LPENR, APB1LLPENR,
            APB1HLPENR, APB2LPENR, APB4LPENR, RESERVED13[4];
...
{
    struct mg_tcpip_driver_stm32h_data* d = (struct
                                             mg_tcpip_driver_stm32h_data*)
                                                ifp->driver_data;
    s_ifp = ifp;

    // Init RX descriptors
    for (int i = 0; i < ETH_DESC_CNT; i++) {
        s_rxdesc[i][0] = (uint32_t)(uintptr_t)
            s_rxbuf[i];                               // Point to data buffer
    ...
    // NOTE(scaprile): We do not use timing facilities so the DMA engine does
    // not re-write buffer address
    ...
    eth_write_phy(PHY_ADDR, PHY_BCR, BIT(15)); // Reset PHY
    eth_write_phy(PHY_ADDR, PHY_BCR, BIT(12)); // Set autonegotiation
    ETH->DMACRDLAR = (uint32_t)(uintptr_t)
        s_rxdesc;                      // RX descriptors start address
    ETH->DMACRDRLR = ETH_DESC_CNT - 1; // ring length
    ETH->DMACRDTPR = (uint32_t)(uintptr_t)&s_rxdesc
        [ETH_DESC_CNT - 1]; // last valid descriptor address
    ETH->DMACTDLAR = (uint32_t)(uintptr_t)
        s_txdesc;                      // TX descriptors start address
    ETH->DMACTDRLR = ETH_DESC_CNT - 1; // ring length
    ETH->DMACTDTPR = (uint32_t)(uintptr_t)
        s_txdesc;    // first available descriptor address
...
void ETH_IRQHandler(void)
{
    if (ETH->DMACSR & BIT(6)) {         // Frame received, loop
        ...
        for (uint32_t i = 0; i < 10;
            ...
            s_rxdesc[s_rxno][3] = BIT(31) | BIT(30)
                | BIT(24); // OWN, IOC, BUF1V
            if (++s_rxno >= ETH_DESC_CNT)
                s_rxno = 0;
        }
    }
    ETH->DMACSR = BIT(7) | BIT(8); // Clear possible RBU RPS while processing
    ETH->DMACRDTPR = (uint32_t)(uintptr_t)&s_rxdesc[ETH_DESC_CNT
                                                    - 1]; // and resume RX
}
...
struct tm4c_emac {
    volatile uint32_t EMACCFG, EMACFRAMEFLTR, EMACHASHTBLH, EMACHASHTBLL,
        ...
        EMACSTATUS, EMACRWUFF, EMACPMTCTLSTAT, RESERVED1[2], EMACRIS, EMACIM,
        EMACADDR0H, EMACADDR0L, EMACADDR1H, EMACADDR1L, EMACADDR2H, EMACADDR2L,
        EMACADDR3H, EMACADDR3L, RESERVED2[31], EMACWDOGTO, RESERVED3[8],
        EMACMMCCTRL, EMACMMCRXRIS, EMACMMCTXRIS, EMACMMCRXIM, EMACMMCTXIM,
        RESERVED4, EMACTXCNTGB, RESERVED5[12], EMACTXCNTSCOL, EMACTXCNTMCOL,
        RESERVED6[4], EMACTXOCTCNTG, RESERVED7[6], EMACRXCNTGB, RESERVED8[4],
        EMACRXCNTCRCERR, EMACRXCNTALGNERR, RESERVED9[10], EMACRXCNTGUNI,
        RESERVED10[239], EMACVLNINCREP, EMACVLANHASH, RESERVED11[93],
        EMACTIMSTCTRL, EMACSUBSECINC, EMACTIMSEC, EMACTIMNANO, EMACTIMSECU,
        EMACTIMNANOU, EMACTIMADD, EMACTARGSEC, EMACTARGNANO, EMACHWORDSEC,
        EMACTIMSTAT, EMACPPSCTRL, RESERVED12[12], EMACPPS0INTVL, EMACPPS0WIDTH,
...
static uint32_t get_sysclk(void)
{
    ...
    uint32_t clk = 0, piosc = 16000000 /* 16 MHz */,
             mosc = 25000000 /* 25MHz */;
...
{
    struct mg_tcpip_driver_tm4c_data* d = (struct mg_tcpip_driver_tm4c_data*)
                                              ifp->driver_data;
    s_ifp = ifp;
    ...
    EMAC->EMACCFG = BIT(2) | BIT(3) | BIT(11)
        | BIT(14); // RE, TE, Duplex, Fast
    EMAC->EMACDMAOPMODE = BIT(1) | BIT(13) | BIT(21)
        | BIT(25); // SR, ST, TSF, RSF
...
static bool mg_tcpip_driver_tm4c_up(struct mg_tcpip_if* ifp)
{
    ...
    if ((ifp->state == MG_TCPIP_STATE_DOWN) && up) { // link state just went up
        ...
        uint32_t emaccfg = EMAC->EMACCFG | BIT(14)
            | BIT(11); // 100M, Full-duplex
        if (sts & BIT(1))
            emaccfg &= ~BIT(14); // 10M
        if ((sts & BIT(2)) == 0)
...
static void mkpay(struct pkt* pkt, void* p)
{
    pkt->pay = mg_str_n((char*)p,
                        (size_t)(&pkt->raw.ptr[pkt->raw.len] - (char*)p));
}
...
static size_t ether_output(struct mg_tcpip_if* ifp, size_t len)
{
    ...
    // size) if (len < min) memset(ifp->tx.ptr + len, 0, min - len), len = min;
...
static void onstatechange(struct mg_tcpip_if* ifp)
{
    if (ifp->state == MG_TCPIP_STATE_READY) {
        ...
        if (ifp->lease_expire > ifp->now) {
            MG_INFO(("       Lease: %lld sec",
                     (ifp->lease_expire - ifp->now) / 1000));
        }
        arp_ask(ifp, ifp->gw);
...
{
    struct ip* ip = tx_ip(ifp, mac_dst, 17, ip_src, ip_dst,
...
static size_t tx_tcp(struct mg_tcpip_if* ifp, uint8_t* dst_mac,
                     uint32_t dst_ip, uint8_t flags, uint16_t sport,
                     uint16_t dport, uint32_t seq, uint32_t ack,
                     const void* buf, size_t len)
{
    struct ip* ip = tx_ip(ifp, dst_mac, 6, ifp->ip, dst_ip,
...
static void read_conn(struct mg_connection* c, struct pkt* pkt)
{
    ...
    if (pkt->tcp->flags & TH_FIN) {
        ...
        // text, we copy to c->recv. If the connection is TLS, this data is
        // encrypted, therefore we copy that encrypted data to the s->raw
        // iobuffer instead, and then call mg_tls_recv() to decrypt it. NOTE:
        // mg_tls_recv() will call back mg_io_recv() which grabs raw data from
        // s->raw
...
static void rx_ip(struct mg_tcpip_if* ifp, struct pkt* pkt)
{
    if (pkt->ip->proto == 1) {
        ...
        } else if (ifp->enable_dhcp_server
                   && pkt->udp->dport == mg_htons(67)) {
            pkt->dhcp = (struct dhcp*)(pkt->udp + 1);
            mkpay(pkt, pkt->dhcp + 1);
        ...
        uint16_t off = (uint16_t)(sizeof(*pkt->ip)
                                  + ((pkt->tcp->off >> 4) * 4U));
        if (iplen >= off)
...
static void mg_tcpip_poll(struct mg_tcpip_if* ifp, uint64_t uptime_ms)
{
    ...
    if (ifp->driver->rx != NULL) { // Polling driver. We must call it
        size_t len = ifp->driver->rx(ifp->recv_queue.buf, ifp->recv_queue.size,
                                     ifp);
        if (len > 0)
    ...
    for (struct mg_connection* c = ifp->mgr->conns; c != NULL; c = c->next) {
        ...
        if (uptime_ms > s->timer) {
            if (s->ttype == MIP_TTYPE_ACK) {
                ...
                tx_tcp(ifp, s->mac, c->rem.ip, TH_ACK, c->loc.port,
                       c->rem.port, mg_htonl(s->seq), mg_htonl(s->ack), "", 0);
            } else {
                if (s->tmiss++ > 2) {
                    ...
                    tx_tcp(ifp, s->mac, c->rem.ip, TH_ACK, c->loc.port,
                           c->rem.port, mg_htonl(s->seq - 1), mg_htonl(s->ack),
                           "", 0);
                }
            }
            settmout(c, MIP_TTYPE_KEEPALIVE);
...
static void send_syn(struct mg_connection* c)
{
    ...
    struct mg_tcpip_if* ifp = (struct mg_tcpip_if*)c->mgr->priv;
    tx_tcp(ifp, s->mac, c->rem.ip, TH_SYN, c->loc.port, c->rem.port, isn, 0,
           NULL, 0);
}
...
void mg_connect_resolved(struct mg_connection* c)
{
    ...
    if (((c->rem.ip & ifp->mask) == (ifp->ip & ifp->mask))) {
        // If we're in the same LAN, fire an ARP lookup. TODO(cpq): handle
        // this!
...
static void close_conn(struct mg_connection* c)
{
    ...
    if (c->is_udp == false && c->is_listening == false) { // For TCP conns,
        struct mg_tcpip_if* ifp = (struct mg_tcpip_if*)
                                      c->mgr->priv; // send TCP FIN
        tx_tcp(ifp, s->mac, c->rem.ip, TH_FIN | TH_ACK, c->loc.port,
...
bool mg_send(struct mg_connection* c, const void* buf, size_t len)
{
    ...
    if (ifp->ip == 0 || ifp->state != MG_TCPIP_STATE_READY) {
        ...
        struct connstate* s = (struct connstate*)(c + 1);
        tx_udp(ifp, s->mac, ifp->ip, c->loc.port, c->rem.ip, c->rem.port, buf,
               len);
        res = true;
    } else {
